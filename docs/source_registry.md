...
| Local hackathon Sentinel-1 and DEM bundles | Local provided files; see `docs/local_data_library.md` and `outputs/sentinel1_selected_file_manifest.csv` | Project owner reported hackathon-provided local data can be used freely for the project; selected standalone Sentinel-1 file has a SHA-256 checksum, but processing still requires provenance, event timing, and reference-mask gates. | Standalone Sentinel-1 TIFF is 2-band VV/VH at `23296 x 23296`; DEM ZIP members are tiled elevation/slope TIFFs. | Filename timestamps are unresolved for Sentinel-1; DEM packages are static context. | Sentinel-1 standalone TIFF overlaps the Mae Sai MVP point; DEM tiles can support terrain and false-positive context. | Selected Sentinel-1 readiness lane exists, but it is not a locked pre/post flood pair and not a reference mask; `processing_allowed=False` until provenance and timing are resolved. |
