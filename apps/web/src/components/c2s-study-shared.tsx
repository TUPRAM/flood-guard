...
export function isStudyVisualUrl(url: string) { return url.startsWith(`${STUDY_ASSETS}visuals/`) && !url.includes("..") && !url.includes("?") && !url.includes("#"); }
...
export function StudyFigure({ layer, title, description, overlay }: { layer?: VisualLayer; title: string; description?: string; overlay?: VisualLayer }) {
  ...
  const overlayAvailable = overlay && isStudyVisualUrl(overlay.url) && overlayFailed !== overlay.url && overlay.width === layer?.width && overlay.height === layer?.height;
  return <figure className={styles.figure}>{available ? <a className={styles.imageStack} href={layer.url} target="_blank" rel="noreferrer"><Image src={layer.url} alt={title} width={layer.width ?? 512} height={layer.height ?? 512} unoptimized loading="lazy" onError={() => setFailed(layer.url)} />{overlayAvailable ? <Image className={styles.overlayImage} style={{visibility:overlayLoaded === overlay.url ? "visible" : "hidden"}} src={overlay.url} alt="Abstention overlay at 35 percent opacity" width={overlay.width ?? 512} height={overlay.height ?? 512} unoptimized loading="lazy" onLoad={() => setOverlayLoaded(overlay.url)} onError={() => setOverlayFailed(overlay.url)} /> : null}</a> : <div className={styles.imageUnavailable}>Preview unavailable for this record.<br />No replacement image is shown.</div>}<figcaption><strong>{title}</strong>{description ? <span>{description}</span> : null}{overlay ? overlayAvailable ? overlayLoaded === overlay.url ? <span className={styles.overlayNote}>Abstention overlay applied at 35% opacity (multiply). Light green: accepted; magenta: abstained; grey: invalid. Turn it off to read the original probability or entropy colours.</span> : <span role="status">Loading the abstention overlay…</span> : <span role="alert">Abstention overlay unavailable or incompatible. The base image is shown without the overlay.</span> : null}{available ? <><VisualLegend layer={layer}/><span>Open the recorded preview · benchmark metrics use full-resolution source pixels.</span></> : null}</figcaption></figure>;
}

export function StudyFrame({ section, children }: { section: StudySection; children: ReactNode }) {
  const mae = section === "mae-sai";
  return <div className={styles.page} lang="en"><StudioLibraryHeader /><main id="main-content" className={styles.main}><nav aria-label="Breadcrumb" className={styles.breadcrumb}><Link href="/studio/">Studies &amp; evidence</Link><span>/</span><Link href={`${STUDY_BASE}/`}>C2S-MS</Link>{section !== "overview" ? <><span>/</span><span>{SECTIONS.find(([id]) => id === section)?.[1]}</span></> : null}</nav><header className={styles.hero}><p className={styles.eyebrow}>{mae ? "Separate inference record · Thailand" : "Public benchmark · Human water reference"}</p><h1>{mae ? "Mae Sai · application of C2S models" : "C2S-MS public benchmark"}</h1><p>{mae ? "Frozen C2S-trained models applied to the August / September 2024 Sentinel-1 pair. Inspect probability, uncertainty and abstention for the recorded acquisition." : "A reproducible study of Random Forest, XGBoost and SAR U-Net across independent events. The target is event-date water, including permanent water."}</p><div className={styles.badges}><span className={styles.badge}>{mae ? "Inference completed" : "Benchmark completed"}</span><span className={styles.badge}>Report only</span><span className={styles.badge}>{mae ? "Thai accuracy unmeasured" : "C2S-MS human labels"}</span></div><dl className={styles.context}><div><dt>{mae ? "Observations · UTC" : "Observation period"}</dt><dd>{mae ? "22 Aug / 15 Sep 2024" : "12 Aug 2016 – 20 Oct 2020"}</dd></div><div><dt>{mae ? "Evaluation status" : "Held-out evaluation"}</dt><dd>{mae ? "No qualified Thai reference" : "Australia · Nigeria · Pakistan"}</dd></div><div><dt>Study / revision</dt><dd>c2s-ms-20260915 · r1</dd></div><div><dt>{mae ? "Model lineage" : "Experiment date"}</dt><dd>{mae ? "Evaluated on C2S-MS; applied to Mae Sai" : "15 September 2026"}</dd></div></dl></header><div className={styles.layout}><nav className={styles.sidebar} aria-label="C2S study sections"><p>Inside this study</p>{SECTIONS.map(([id, label, path], i) => <Link key={id} href={`${STUDY_BASE}${path}/`} aria-current={section === id ? "page" : undefined}><span>{String(i + 1).padStart(2, "0")}</span>{label}</Link>)}</nav><div className={styles.content}><label className={styles.mobileNav}>Study section<select value={section} onChange={(e) => { const item = SECTIONS.find(([id]) => id === e.target.value); if (item) window.location.assign(`${STUDY_BASE}${item[2]}/`); }}>{SECTIONS.map(([id, label]) => <option value={id} key={id}>{label}</option>)}</select></label>{children}<footer className={styles.footer}>C2S-MS · revision r1 · research report only. These outputs cannot feed exposure, road risk, access, equity or FPPS. FloodGuard supports preparedness and rapid post-event prioritisation; this study is not an official warning.<br /><Link href="/studio/">All studies</Link> · <Link href="/studio/planning-evidence/">Planning evidence</Link> · <Link href="/studio/archive/mae-sai-geoai/">Historical research</Link></footer></div></div></main></div>;
}
