...
| Academic or manual reference mask | Chiang Rai / Mae Sai 2024 and Hat Yai / Songkhla 2025 | Fallback validation or manual reference candidate | `docs/manual_reference_mask_protocol.md` | Mae Sai cross-border GeoPackage exists outside Git and is checksum-tracked; it has no Thailand reporting-area overlap; no Hat Yai manual file is recorded | project-owned weak-reference candidate; not official validation truth | reference_only; source GeoPackage remains outside Git | cite as FloodGuard manual weak-reference candidate with source-basis and cross-border scope notes | current Mae Sai geometry is cross-border only; Hat Yai file is absent; weak-reference status does not clear official validation or ML-label gates | retain the existing file only for cross-border candidate metrics; any new in-area candidate needs a distinct identity and full inspection, rights, authority, review, and release gates |
...
| Academic or manual reference mask | mae_sai_cross_border_file_ready_hat_yai_missing | not_applicable | no_provider_response | mae_sai_cross_border_geometry_available_hat_yai_unresolved | candidate_metrics_cross_border_only | candidate_metrics_cross_border_only | candidate_demo_cross_border_only | reference_only | yes_project_notes | no | blocked_not_in_area_not_official_truth_and_no_hat_yai_file | keep the Mae Sai file limited to cross-border candidate calibration; create a distinct in-area candidate only under the full protocol and keep official validation/ML-label gates blocked |
