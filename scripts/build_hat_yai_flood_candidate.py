"""Derive the Hat Yai fixed-rule amplitude-change candidate from verified originals."""
...
def main() -> None:
    ...
    aoi_path = root / "resources/aoi/upload/aoi-03_hat_yai_core.geojson"
    ...
    manifest_path = root / "outputs/cdse_hat_yai_acquisition_manifest.csv"
    rows = list(csv.DictReader(manifest_path.open(encoding="utf-8-sig")))
    selected = {}
    for date, role in (("2025-11-11", "pre"), ("2025-11-23", "post")):
    ...
    # Transfer the existing Mae Sai exploratory rule without Hat Yai tuning.
    ...
    metadata = {
        ...
        "rights": {
            ...
            "attribution": "Contains modified Copernicus Sentinel data (2025), processed by FloodGuard.",
        },
        "limitations": [
            "Amplitude-drop candidate, not validated inundation or a calibrated flood probability.",
            "The fixed Mae Sai exploratory threshold is transferred without Hat Yai calibration; applicability remains unverified.",
            ...
            "Observation question: change from 11 to 23 November 2025 UTC (12 to 24 November Thailand), not the event peak or all floodwater.",
