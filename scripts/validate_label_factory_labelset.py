"""Validate a frozen labelset against content, QA, lineage, and raster hashes."""
...
def main() -> None:
    """Recompute every external hash and return a compact validation receipt."""
    ...
    parser.add_argument("--raster", action="append", required=True, metavar="NAME=PATH")
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        type=Path,
        required=True,
        help="New immutable self-hashed validation receipt JSON.",
    )
    ...
    try:
        ...
        write_labelset_validation_receipt(receipt, args.receipt_output)
    except (
        ReviewWorkflowError,
        ReviewerCalibrationError,
        AnnotationValidationError,
    ...
    print(f"VALID: {receipt.labelset_id}")
    print(f"Manifest SHA-256: {receipt.manifest_sha256}")
    print("Open adjudications: 0")
    print(f"Wrote validation receipt: {args.receipt_output}")
