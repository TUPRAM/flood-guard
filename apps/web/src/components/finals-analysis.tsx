...
export const SERVICE_NAMES: Record<FinalsServiceId, [string, string]> = {
  hospital: ["Hospital care", "โรงพยาบาล"], primary_care: ["Primary care", "บริการปฐมภูมิ"],
  pharmacy: ["Pharmacy", "ร้านขายยา"], shelter: ["Evacuation shelter", "ศูนย์พักพิง"],
};
...
function InterventionTable({ items, th, detailed = false }: { items: FinalsIntervention[]; th: boolean; detailed?: boolean }) {
  return <div className={styles.tableWrap} tabIndex={0} role="region" aria-label={th ? "ผลการทดลองการเข้าถึง เลื่อนแนวนอนได้" : "Access experiments; scroll horizontally"}>
    <table><thead><tr><th>{th ? "การเปลี่ยนแปลง" : "Change"}</th>{[15, 30, 60].map((minutes) => <th key={minutes}>{minutes} {th ? "นาที: เพิ่ม / สูญเสีย" : "min: gain / lose"}</th>)}<th>{th ? "มีเส้นทางเพิ่ม / ไม่มีเส้นทาง" : "Gain any route / lose all routes"}</th><th>{th ? "เวลาเฉลี่ยเปลี่ยน (นาที)" : "Mean change (minutes)"}</th>{detailed ? <><th>{th ? "P90 / สูงสุด (นาที)" : "P90 / max change (minutes)"}</th><th>{th ? "คน-นาทีสุทธิ" : "Net person-minutes"}</th></> : null}</tr></thead>
      <tbody>{items.map((item) => <tr key={item.id}><th scope="row">{KINDS[item.kind][th ? 1 : 0]}<small>{item.target_label}</small><details><summary>{th ? "เหตุผลและรหัสการทดสอบ" : "Selection and identity"}</summary><p>{item.selection_method}</p><p>{item.id} · {item.target_id}</p></details></th>{[15, 30, 60].map((minutes) => <td key={minutes}>{n(item[`gaining_${minutes}_min_access` as "gaining_15_min_access"], th)} / {n(item[`losing_${minutes}_min_access` as "losing_15_min_access"], th)}</td>)}<td>{n(item.newly_reachable_population, th)} / {n(item.newly_unreachable_population, th)}</td><td>{delta(item.mean_travel_time_delta_minutes, th)}<small>{th ? "ประชากรที่เปรียบเทียบได้" : "Comparable residents"}: {n(item.comparable_population, th)}</small></td>{detailed ? <><td>{delta(item.p90_delta_minutes, th)} / {delta(item.max_delta_minutes, th)}</td><td>{delta(item.net_person_minutes, th)}</td></> : null}</tr>)}</tbody>
    </table>
  </div>;
}
...
export function FinalsAnalysisPanel({ analysis, layers, th, context }: { analysis: FinalsAnalysis; layers: EvidenceLibraryLayer[]; th: boolean; context?: ReactNode }) {
  ...
  return <div data-finals-analysis="true" className={styles.finalsWorkspace}>
    ...
    <dialog ref={dialog} id={detailsId} className={styles.workspaceDialog} data-finals-detail-panel="true" aria-labelledby={`${detailsId}-title`}>
      ...
      <details><summary>{th ? "ลำดับเวลาของหลักฐานน้ำท่วม" : "Flood evidence timeline"}</summary><ol>{analysis.timeline.map((item) => <li key={item.id}><strong>{item.title}</strong><p>{item.start ?? "—"} → {item.end ?? "—"} · {item.role}</p><ul>{item.limitations.map((point) => <li key={point}>{point}</li>)}</ul><a href={item.source_url} target="_blank" rel="noopener noreferrer">{th ? "แหล่งข้อมูล" : "Source"}</a></li>)}</ol></details>
      {analysis.topology_review ? <details><summary>{th ? "การตรวจสอบจุดเชื่อมถนน" : "Road-connection review"}</summary><p>{analysis.topology_review.summary}</p><p>{th ? "รายการตรวจสอบ / จุดเชื่อมที่ยอมรับ" : "Reviewed candidates / accepted connections"}: {analysis.topology_review.reviewed_candidates} / {analysis.topology_review.accepted_connections}</p></details> : null}
      <details><summary>{th ? "ตัวตนและบทบาทของจุดหมาย" : "Destination identities and roles"}</summary><p>{th ? "การเปิดใช้งานขณะเกิดเหตุและความจุจริงยังไม่ทราบ พิกัดพื้นที่อาคารไม่ใช่ทางเข้าที่ตรวจสอบแล้ว" : "Event-time availability and actual capacity remain unknown. A site coordinate is not a verified entrance."}</p><div className={styles.tableWrap} tabIndex={0} role="region" aria-label={th ? "ตารางสถานที่ เลื่อนแนวนอนได้" : "Destination review; scroll horizontally"}><table><thead><tr><th>{th ? "สถานที่" : "Location"}</th><th>{th ? "บริการ" : "Service"}</th><th>{th ? "บทบาทเรขาคณิต" : "Geometry role"}</th><th>{th ? "เข้าข่ายการทดสอบ" : "Scenario eligible"}</th></tr></thead><tbody>{analysis.facility_review.map((item) => <tr key={item.id}><th scope="row"><a href={item.source_url} target="_blank" rel="noopener noreferrer">{item.name}</a><small>{item.id}</small></th><td>{SERVICE_NAMES[item.service_type as FinalsServiceId]?.[th ? 1 : 0] ?? item.service_type}</td><td>{item.geometry_role}</td><td>{item.eligible ? (th ? "ใช่ — ยังไม่ยืนยันการใช้งานจริง" : "Yes — operation unverified") : (th ? "ไม่ใช่" : "No")}</td></tr>)}</tbody></table></div></details>
