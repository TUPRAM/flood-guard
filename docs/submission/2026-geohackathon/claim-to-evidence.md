...
| C02 | The platform has role-specific `/public`, `/command`, and `/studio` routes. | `apps/web/src/app/`, route smoke tests and six checksummed screenshots. | Implemented and visually verified at all required viewports | May say the routes exist and work with fixtures. |
| C03 | The web demo works without a hosted API or external network. | Reproducible offline bundle, service worker, static export, offline browser smoke and bundle-integrity tests. | Implemented and verified with zero external runtime requests; final extracted-ZIP review remains a release checklist gate | May say "offline fixture demo"; never imply live data. |
| C04 | FastAPI exposes versioned artifact and deterministic scenario contracts. | `services/api/src/floodguard_api/app.py`, Pydantic models and the 86-test API receipt. | Implemented and tested | May say tested contract layer; do not imply the proposal demo requires a hosted API. |
...
| C10 | The real GeoAI smoke invokes tile export and tiled inference. | `services/geoai-runner/tests/test_geoai_smoke.py`, checksummed proof receipt and opt-in JUnit receipt. | Implemented and passed on the tested source commit | May name `export_geotiff_tiles`, model construction and `predict_geotiff`. |
...
- Do not show synthetic perfect metrics from fixture contracts as model
  performance.
- Replace the old `248 passing tests` sentence with the exact fresh,
  commit-bound test receipt in `evidence/test-results.json`.
