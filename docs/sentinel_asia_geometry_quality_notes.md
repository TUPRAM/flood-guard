...
- Do not use this Sentinel Asia reference candidate for qualified validation or as a baseline reference until its own manifest row passes without `--allow-blocked`. The separate manual cross-border weak-reference calibration does not clear this source's gates.
