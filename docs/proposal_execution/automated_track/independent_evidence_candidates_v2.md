...
**Finding:** a dated GISTDA ALOS-2 flood-vector candidate was located in Mae Sai through the signed-in STAC map, as detailed below. A complete AOI export, processing description and reuse grant have not been obtained or verified. It is therefore not a current pixelwise reference or an accuracy result. This does not change either frozen automated two-method agreement experiment.
...
[GISTDA's historical flood STAC page](https://disaster.gistda.or.th/services/stac/flood/index) requires login for the full catalogue. Its [public API documentation](https://disaster.gistda.or.th/services/open-api) advertises only rolling 1-, 3-, 7-, and 30-day flood extent endpoints. A later signed-in Chrome inspection located the 15 September 2024 collection/item `flood_r2_20240915` and its GeoJSON **Features** asset. The asset reports `numberMatched=15,567` across the day but returns only ten features on its first page; the ten first-page polygons were not a complete spatial search. The map popup for one flood polygon identifies `file_name=ALOS2_20240915_0026`, district Mae Sai, and displayed latitude/longitude **20.4306472802, 99.9142217811**, within the AOI-01 bounding box. This establishes a promising same-day, different-sensor *candidate*, not an exported or validated AOI reference. The portal's download listing also shows the ALOS-2 product, but its visible downloads were cartographic images and a spreadsheet, without an AOI vector export. Its title says 00:26 while its description says 00:22; a time zone, complete footprint, class semantics, processing lineage and reuse terms remain unverified. Do not copy the logged-in asset URL or its embedded access token into a receipt, document or message.

Request an AOI-clipped GeoJSON, GeoPackage or zipped Shapefile from GISTDA for `flood_r2_20240915`, restricted to `file_name=ALOS2_20240915_0026` and bbox `[99.8384,20.3705,99.9441,20.4563]`. [GISTDA's platform overview](https://www.gistda.or.th/news_view.php?lang=EN&n_id=8639) describes Shapefile downloads generally, although this ALOS-2 listing did not visibly offer one. Preserve the full source geometry, feature IDs, acquisition time and time zone, sensor/product provenance, processing method, completeness/unknown-area semantics, and written permission for validation, derivatives and publication. If supplied, hash the original export outside Git and decide whether it can support a separate exploratory comparison; neither v1 nor v2 hold-out can be rescored as a new confirmatory result.

The official [GISTDA historical flood dataset](https://opendata.gistda.or.th/dataset/flood-disaster-data) and its [STAC resource record](https://opendata.gistda.or.th/dataset/flood-disaster-data/resource/b5006f9f-ca4f-45ac-bcd5-dc2a609b1c7a) state **“License not specified.”** The dataset lists `disaster@gistda.or.th` as contact. A separate [rolling flood-extent API](https://opendata.gistda.or.th/en/dataset/disasters-03) has an “Open Data Common” label; that label cannot be transferred to the historical STAC asset. Request the export and explicit validation, derivative and publication terms from the named contact if the portal does not expose them.
